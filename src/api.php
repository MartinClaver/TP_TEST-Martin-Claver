...
try {
    if ($method === 'POST' && isset($_POST['name'], $_POST['email'], $_POST['role'])) {
        $userManager->addUser($_POST['name'], $_POST['email'], $_POST['role']);
        ...
        if (isset($_PUT['id'], $_PUT['name'], $_PUT['email'])) {
            $userManager->updateUser($_PUT['id'], $_PUT['name'], $_PUT['email'], $_PUT['role']);
